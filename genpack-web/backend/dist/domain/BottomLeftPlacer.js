...
exports.BottomLeftPlacer = void 0;
...
class BottomLeftPlacer {
    constructor(stockWidth) {
        this.ly = 0;
        this.pivots = [];
        this.chosen = null;
        if (stockWidth <= 0)
            throw new Error('stockWidth must be positive');
    ...
    seedPivots() {
        ...
        for (let i = 0; i < n; i++) {
            this.pivots.push(new Pivot_1.Pivot(0, this.ly + i, this.stockWidth, 2 * this.stockWidth - i, 2));
        }
    }
     ...
     */
    checkx(placedRect, chosenPivot, newPivot) {
        const p = this.pivots;
     ...
     */
    checky(placedRect, chosenPivot, newPivot) {
        const p = this.pivots;
        const count = p.length;
        const rightX = chosenPivot.sx + placedRect.length;
    ...
    ch(pPos, rePos, orderedRects) {
        ...
        if (!parentRect || !pforRect)
            return;
        if (parentRect.width !== pforRect.width)
            return;
        const newP = new Pivot_1.Pivot(p[pPos].sx + placedRect.length, p[pPos].sy, p[pPos].avalx - placedRect.length, p[pPos].avaly, 0);
        newP.parent = rePos;
        newP.ischeckx = p[pPos].ischeckx;
        newP.ischecky = p[pPos].ischecky;
        this.pivots.push(newP);
    }
    pivotStatus(pflag, pPos, rePos, orderedRects) {
        const p = this.pivots;
        const chosenPivot = this.chosen;
        const placedRect = orderedRects[rePos];
        const parentRect = orderedRects[p[pPos].parent];
        const pushPivot = (sx, sy, avalx, avaly, which, parent) => {
            const n = new Pivot_1.Pivot(sx, sy, avalx, avaly, which);
            n.parent = parent;
        ...
        switch (pflag) {
            case 0: {
                pushPivot(p[pPos].sx + placedRect.length, p[pPos].sy, p[pPos].avalx - placedRect.length, p[pPos].avaly, 0, rePos);
                this.checky(placedRect, chosenPivot, lastPivot());
                pushPivot(p[pPos].sx, p[pPos].sy + placedRect.width, p[pPos].avalx, p[pPos].avaly - placedRect.width, 1, rePos);
                break;
            }
            case 1: {
                pushPivot(p[pPos].sx + placedRect.length, p[pPos].sy, p[pPos].avalx - placedRect.length, p[pPos].avaly, 0, rePos);
            ...
            case 5: {
                ...
                pushPivot(p[pPos].sx + placedRect.length, p[pPos].sy, p[pPos].avalx - placedRect.length, p[pPos].avaly, 0, rePos);
                this.checky(placedRect, chosenPivot, lastPivot());
                pushPivot(p[pPos].sx, p[pPos].sy + placedRect.width, p[pPos].avalx, p[pPos].avaly - placedRect.width, 2, rePos);
                this.checkx(placedRect, chosenPivot, lastPivot());
                break;
            }
        }
    ...
    place(orderedRects) {
        ...
        for (let is = 0; is < orderedRects.length; is++) {
            ...
            if (this.pivots[k].which === 0) {
                ...
                if (parentW > rect.width)
                    pflag = 0;
                else if (parentW === rect.width)
                    pflag = 1;
                else
                    pflag = 2;
            }
...
exports.BottomLeftPlacer = BottomLeftPlacer;
//# sourceMappingURL=BottomLeftPlacer.js.map